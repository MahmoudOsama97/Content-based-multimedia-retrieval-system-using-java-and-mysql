...
public class InsertImage {
    ...
    public void run(String args , Connection conn, String title, String info) throws SQLException {
        ...
        try {
            Q = "insert into images(id,title,url,info) values("+ String.valueOf(idMax+1) + ", " + "\""+title+ "\"" + ", " + "\""+args.replace("\\", "\\\\") + "\""  + ", " +  "\""+info+ "\""+ ")" ;
            System.out.println(Q);
            Statement stmt = conn.createStatement();
            stmt.executeUpdate(Q);
        }catch (SQLException ex) {
        ...
        meanQuery( conn , src, rHist, gHist, bHist,idMax+1);
        ...
        Q +=" where id="+String.valueOf(idMax+1);
        System.out.println(Q);
        try{
            Statement stmt = conn.createStatement();
            stmt.executeUpdate(Q);
